...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=str,
        required=True,
        help="Path to an existing run folder (contains prompts.jsonl).",
    )
    ap.add_argument("--device", type=str, default="cuda" if torch.cuda.is_available() else "cpu")
    ap.add_argument("--dtype", type=str, default="bfloat16")
    ap.add_argument("--seed", type=int, default=0)
    ap.add_argument(
        "--layers", type=str, default="4,12,20,28", help="Comma-separated layer ids to capture."
    )
    ...
    for r in prompt_rows:
        ...
        tensor_payload = {
            ...
            "attention_mask": inputs.get("attention_mask")[0].detach().to("cpu")
        ...
        for lid in layers:
            ...
            if la.mlp_in is None or la.mlp_out is None:
                raise RuntimeError(
                    f"Missing activations for layer {lid}. Hook path may be wrong for this model."
                )
