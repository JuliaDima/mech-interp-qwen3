...
#   2. coordinator  (GPU,  5 min)  — reads emergence.npy, selects top-4 anchors,
#                                    submits one anchor_pipeline job per anchor
#
# Each anchor_pipeline job (GPU, 60 min) then runs:
#   run_concept + null permutation + transcoder sweep sequentially.
#
...
#   bash scripts/submit_full_pipeline.sh --dry-run          # preview only
#   bash scripts/submit_full_pipeline.sh --dry-run carry    # preview for carry
#
# Optional env overrides
# ----------------------
#   GIF_N=50            pairs per template for make_gif    (default: 100)
#   ANCHOR_K=4          number of anchors to select        (default: 4)
#   ANCHOR_TIME=01:30:00  time limit per anchor pipeline   (default: 01:00:00)
...
GIF_N="${GIF_N:-100}"
ANCHOR_K="${ANCHOR_K:-4}"
ANCHOR_TIME="${ANCHOR_TIME:-01:00:00}"
N_PAIRS="${N_PAIRS:-100}"
NULL_K="${NULL_K:-20}"
