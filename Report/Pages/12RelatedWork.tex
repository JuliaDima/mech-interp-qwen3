...
are one-dimensional, motivating the use of other localisation methods beyond rank-one directions.
...
prefix vectors shift model behaviour without modifying weights. Whether such interventions 
succeed likely depends on the underlying representational geometry of the task in question.

The present work asks whether mechanistic structure comparable to that found in small or
proprietary models is recoverable in \texttt{Qwen3-4B-Instruct}. Beyond reproducibility,
the contrastive concept-localisation method introduced here provides additional insight
into how features encode abstract computation, characterising the geometric structure
through which an instruction-tuned model represents arithmetic reasoning across layers
and token positions.