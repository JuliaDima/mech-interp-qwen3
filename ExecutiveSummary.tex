...
\usepackage{amssymb}
\usepackage{graphicx}
\graphicspath{{MPhilThesisReport/Pages/Figures/}}
\usepackage{booktabs}
\usepackage{xcolor}
\usepackage{titlesec}
\usepackage{parskip}
...
\textbf{GCD divisibility} shows a qualitatively different structure: the inter-layer cosine matrix exhibits a sharp block transition at a specific layer, indicating that the concept direction \emph{rotates} rather than persists. Concept-relevant features are localisable but more distributed across transcoder feature space than for carry. Figure~\ref{fig:gcd_summary} illustrates the concept summary for GCD at the primary anchor, including the top transcoder features projected along the delta direction.

\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{gcd/concept_summary_T0_thesis.pdf}
  \caption{\small Concept summary for GCD divisibility ($\gcd(a,7)=7$) at the primary anchor. Each panel shows a top transcoder feature's activation pattern projected along the contrastive delta direction, with positive values (blue) indicating features that activate more strongly on divisible inputs.}
  \label{fig:gcd_summary}
\end{figure}
