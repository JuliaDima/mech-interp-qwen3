...
Fourier structure in transcoder feature grids
--------------------------------------------------

The combined cosine score in the previous section ranks features by aggregate alignment
with the concept delta, but says nothing about a feature's finer-grained response shape.
For carry, at the ones\ :sub:`b` anchor, every active feature's post-ReLU activation is
instead binned into a :math:`10\times10` grid over the two operands' ones digits,

.. math::

   X_{l,f}[a_0, b_0] = \mathbb{E}\bigl[z_{l,f}(x) \mid a \bmod 10 = a_0,\; b \bmod 10 = b_0\bigr],

min-max normalised to :math:`[0, 1]` per feature. Writing :math:`\hat X_{l,f}[u,v]` for the
mean-subtracted 2D FFT of this grid, each frequency pair corresponds to a cosine mode
:math:`\cos(2\pi(ua_0 + vb_0)/10 + \varphi_{u,v})`, and modes are grouped into six
symmetry classes according to which of :math:`u, v` are zero or at the Nyquist frequency
:math:`5`: *row*- or *column*-only (a function of a single digit), *row-* or
*column-parity* (:math:`(-1)^{a_0}` or :math:`(-1)^{b_0}`), global *parity*
(:math:`(-1)^{a_0+b_0}`), *iso-sum* (a function of :math:`a_0 + b_0`), *iso-difference* (a
function of :math:`b_0 - a_0`), or *mixed*. The reconstruction from the top-:math:`K` modes
by amplitude is

.. math::

   \hat X_{l,f}^{(K)}[a_0, b_0] = \bar X_{l,f} + \sum_{k=1}^K A_k \cos\!\left(\frac{2\pi(u_k a_0 + v_k b_0)}{10} + \varphi_k\right),

and the *structured-energy fraction* of a feature is the share of its non-DC spectral power
carried by modes with one of the five explicit digit-pair symmetries above (excluding
mixed),

.. math::

   S_{l,f} = \frac{\sum_{(u,v)\,\in\,\{\text{parity, row/col-par., iso-sum, iso-diff.}\}} |\hat X_{l,f}[u,v]|^2}
                  {\sum_{(u,v) \neq (0,0)} |\hat X_{l,f}[u,v]|^2}.

Scanning all active features at every layer and retaining those with :math:`S_{l,f} > 0.6`
concentrates the selected set almost entirely in layers 12–14 and 21 — at or beyond the
layer where the carry delta trajectory reaches its maximum. The table below lists eight
representative features from this set together with the closed-form digit predicate each
appears to encode, each reconstructed to :math:`R^2 \geq 0.90` with at most six Fourier
modes.

.. list-table:: Symmetry category of eight Fourier-structured carry features and the
   arithmetic predicate each appears to encode.
   :widths: 15 10 25 30
   :header-rows: 1
   ...
   * - F\ :sub:`30390`
     - 21
     - column-parity
     - :math:`a_0 \bmod 2 = 1` and :math:`b_0 \bmod 2 = 1`
   * - F\ :sub:`57984`
     - 13
     - iso-difference
     - :math:`a_0 - b_0 = 0`
   ...
   * - F\ :sub:`132660`
     - 13
     - iso-sum
     - :math:`a+b \bmod 10 \in \{4,5,6\}`

This structure invites comparison with the Fourier circuits reported in small transformers
trained from scratch on modular arithmetic: Nanda et al. (2023) find that a one-layer
transformer trained purely on modular addition converges onto a small shared set of
low-frequency components, and Zhong et al. (2023) show that the resulting *clock*
algorithm reads the sum directly off the phase of one or two dominant frequencies. In both
accounts the structure lives in the network's *weights* — the token embeddings organise
into circles at a handful of shared frequencies, and every unit's response follows from
reading out those fixed directions. The structure found here instead lives in
*activation* space: individual transcoder features happen to respond to the operand grid
as approximately single Fourier modes, so the basis is an empirical property of the
dictionary's activation pattern on this task rather than of the weights that produced it.
Consistent with this being an incidental rather than load-bearing property, jointly
ablating the 60 features with the highest structured-energy fraction leaves task accuracy
essentially unchanged (:math:`\Delta p = +0.003`): Qwen3-4B exhibits Fourier structure in
its response to this addition dataset, but that structure appears to be one descriptive
regularity among many rather than a causal component of how the model actually computes
the carry.
