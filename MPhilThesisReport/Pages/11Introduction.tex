...
\begin{flushright}
\emph{``There is a way of grasping a rule which is \textnormal{not} an interpretation.''}\\
...
\textit{Activation patching} establishes causal claims~\citep{conmy2023automated}. Given an original prompt
...
that the patched representation is causally sufficient to alter the prediction. 
...
or sparse feature activations while leaving the remaining computation unchanged. 
The resulting logit difference can then be attributed to a more specific internal component.

\textit{Steering vectors} are extracted by contrasting two prompt sets, $D_+$ where a concept is
...
approximates the axis along which the concept is encoded at depth $\ell$.  Thus, injecting
$\alpha_{\mathrm{steer}}\,\delta^{(\ell)}$ where \(\alpha_{\mathrm{steer}} \in \mathbb{R}\) is a scalar intervention strength, provides a
continuous lever on the concept representation without modifying model weights.  The norm magnitude
$\lVert\delta^{(\ell)}\rVert$ across layers is the main indicator used for concept
localisation throughout this thesis.
...
but as mechanistic tests of whether the analogous sparse features, graph structure, 
and causal interventions transfer to a different model family.
